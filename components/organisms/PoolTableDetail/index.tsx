...
export default function PoolTableDetail() {
    ...
    const getDataRoom = (id: number) => {
        fetchCustom<any>(`${process.env.NEXT_PUBLIC_URL}/hotels/${hotelID}/pool-tables/${id}`, bearerToken)
            .then((result) => {
                if (result.error) {
                    throw new Error(result.data.message);
                } else {
    ...
    const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
        ...
        if (detailRoomID > 0) {
            url = `${process.env.NEXT_PUBLIC_URL}/hotels/${hotelID}/pool-tables/${detailRoomID}`;
        }
